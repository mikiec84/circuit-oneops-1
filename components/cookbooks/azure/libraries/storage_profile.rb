  ...
  class StorageProfile
    ...
    def create_storage_account(storage_account_name)
      if storage_name_avail?(storage_account_name)
        ...
        storage_account = @storage_client.storage_accounts.create(name: storage_account_name,
    ...
    def create_storage_account_name
      ...
      generated_name = generated_name.slice!(0..21) if generated_name.length > 22
      ...
      OOLog.fatal('No storage account can be selected!') if storage_index < 0
    ...
    def generate_storage_account_names(storage_account_name)
      ...
      (1..limit).each do |index|
        storage_accounts[index - 1] = if index < 10
                                        "#{storage_account_name}0#{index}"
                                      else
                                        "#{storage_account_name}#{index}"
                                      end
      end
    ...
    def calculate_storage_index(storage_accounts, vm_count)
      ...
      (0..storage_count).each do |storage_index|
        ...
        return storage_index if vm_count <= storage_account
      end
      -1
    end
    ...
    def storage_name_avail?(storage_account_name)
      begin
        response = @storage_client.storage_accounts.check_name_availability(storage_account_name, 'Microsoft.Storage/storageAccounts')
    ...
    def storage_account_created?(storage_account_name)
      begin
        response = @storage_client.storage_accounts.check_storage_account_exists(@resource_group_name, storage_account_name)
